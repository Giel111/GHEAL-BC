...
LifeExpAndCancerAge <- function(){
  ...
  random_cancer <- 2
  random_life_exp <- 1
  if (runif(1)<(17)){ # add a / between 1 and 7 ;)
...
TreatCostTable <- TreatCostTable * 1
CostDiscount <- 0.04
...
ggplot(data.frame(x), aes(x)) +
  ...
  geom_segment(aes(x = ci_x[1], y = 0, xend = ci_x[2], yend = 0), 
               color = 'blue', size = 1.5) +
...
df_long <- tidyr::pivot_longer(subdf7, cols = c(3:11),
                               names_to = "Column", values_to = "Value")

# create the line plot
p4<-ggplot(data = df_long, aes(x = Column, y = Value, 
                               group = WorstStage, color = factor(WorstStage))) +
  geom_line() +
  scale_color_manual(values = c("black", "blue", "red", "green", "purple")) +
  labs(x = "Time (years)", y = "Relative survival", color = "Stage",
       title='Relatieve overleving voor patiënten gediagnosticeerd met invasieve borstkanker naar stadium bij diagnose') 
plot4 <- recordPlot()
plot.new()
...
CostUtilDf <- df %>%
  dplyr::select(PatientAge,TotalUtility,TotalCosts,MedicalCosts,SocietalCosts,PAIDCosts) %>%
...
meanLY <- mean(CostUtilDf$PatientAge/year,na.rm=T)
# maybe add overhead? 
...
cat(paste0(strrep("_", 50),'\n',
           'Important Input Vars', '\n',
           strrep("_", 50),'\n',
           'Patients Generated: ', n.patients,'\n',
           'Tumor Growth inputs gamma(shape,rate): ',Grow_Gamma_shape,'  ',
           Grow_Gamma_rate,'\n',
           'Screen Detection Inputs lognorm(mean,sd): ',MeanMinDetSize,'  ',
           SDMinDetSize))
